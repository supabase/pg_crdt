...
static void autochange_free_context_callback(void *);

/* Expanded Object Header "methods" for flattening for storage */
static Size autochange_get_flat_size(ExpandedObjectHeader *eohptr);

static void autochange_flatten_into(ExpandedObjectHeader *eohptr, void *result,
                                    Size allocated_size);

static const ExpandedObjectMethods autochange_methods = {
    autochange_get_flat_size, autochange_flatten_into};
...
autochange_Autochange *new_expanded_autochange(MemoryContext parentcontext,
                                               uint8_t const *flat_data,
                                               size_t flat_size) {
  ...
  objcxt = AllocSetContextCreate(parentcontext, "expanded autochange",
                                 ALLOCSET_DEFAULT_SIZES);

  /* Allocate a new expanded autochange */
  change = (autochange_Autochange *)MemoryContextAlloc(
      objcxt, sizeof(autochange_Autochange));
  ...
  AMitemToChange(AMstackItem(&change->stack,
                             AMchangeFromBytes(flat_data, flat_size), _abort_cb,
...
static void autochange_free_context_callback(void *ptr) {
