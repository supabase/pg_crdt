...
Datum autodoc_from_jsonb(PG_FUNCTION_ARGS) {
	...
	if (JsonbIteratorNext(&it, &v, false) != WJB_BEGIN_OBJECT)
		ereport(ERROR,
				(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
				 errmsg("must call %s with an object",
						"autodoc_from_jsonb")));
	...
	AMstackItem(NULL,
				AMcommit(doc->doc, AMstr("autodoc_from_jsonb"), NULL),
				abort_cb,
	...
	while((r = JsonbIteratorNext(it, &v, false)) != WJB_DONE) {
		switch(r) {
			...
			case WJB_BEGIN_OBJECT: {
				newobjid = AMitemObjId(AMstackItem(&doc->stack,
			...
			case WJB_VALUE: {
				switch(v.type) {
					...
					case jbvNumeric: {
						...
						int32 scale = DatumGetInt32(DirectFunctionCall1(numeric_scale, num_datum));

						if (scale == 0) {
							int64 int_value = DatumGetInt64(DirectFunctionCall1(numeric_int8, num_datum));
							AMstackItem(NULL,
