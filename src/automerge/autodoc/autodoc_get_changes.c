...
Datum autodoc_get_changes(PG_FUNCTION_ARGS) {
  ...
  if (SRF_IS_FIRSTCALL()) {
    ...
    changes = AMstackItems(&doc->stack, AMgetChanges(doc->doc, NULL), _abort_cb,
                           AMexpect(AM_VAL_TYPE_CHANGE));

    funcctx->max_calls = AMitemsSize(&changes);
    state->changes =
        palloc(sizeof(autochange_Autochange *) * funcctx->max_calls);

    for (int i = 0; (item = AMitemsNext(&changes, 1)) != NULL; i++) {
      AMitemToChange(item, &itemchange);
      bs = AMchangeRawBytes(itemchange);
      state->changes[i] =
          new_expanded_autochange(CurrentMemoryContext, bs.src, bs.count);
    }
